    ...
    if( isset($btn_custom) && !empty($btn_custom) ):
        foreach($btn_custom as $i => $btn):
			...
			if($button_style == 'btn-fullcolor btn-custom-color' && $button_color ){
				...
				echo $button_selector.':hover{background-color:'.adjustBrightness($button_color, 20).'}';
			}
			// Button lined
			elseif($button_style == 'btn-lined btn-custom-color' && $button_color ){
				...
				echo $button_selector.':hover{color:'.adjustBrightness($button_color, 20).'; border-color:'.adjustBrightness($button_color, 20).';}';
			}
			// Button Skewed
			elseif($button_style == 'btn-fullcolor btn-skewed btn-custom-color' && $button_color ){
				...
				echo $button_selector.':hover:before{background-color:'.adjustBrightness($button_color, 20).';}';
			}
...
if(strpos($header_style_color, 'rgba') === false ){
	...
	.site-header.style8 .kl-top-header {background:<?php echo $kl_top_header_color; ?>;}
	.site-header.style8 .kl-main-header {background:<?php echo $kl_main_header_color; ?>;}
...
if(!empty($menu_font)){
	...
	echo '.mainnav--pointer-dash.main-nav > ul > li.menu-item-has-children > a:after {bottom: calc(52% - '.($mf_fsize-2).');}';
	// Color
	if(!empty($mf_color)){
		echo '.site-header[class*="sh--"] .main-nav > ul > li > a {color:'.$mf_color.'}';
	}
}
