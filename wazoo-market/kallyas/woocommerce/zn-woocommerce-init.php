			...
			$placeholder_height = 0 )
		{
			...
			if ( has_post_thumbnail($post->ID) ) {
                ...
                if(!empty($imageUrl)){
                	...
                	$img = '<img class="kw-prodimage-img" '.$src.' alt="'.esc_attr( get_post_meta($thumbid, '_wp_attachment_image_alt', true) ).'" title="'.esc_attr( get_the_title($thumbid) ).'">';
                }
				...
				if ( $attachment_ids && $use_second_image ) {
					...
					if(!empty($secondary_image_id)){
						...
						if(!empty($image2Url)){
							$img2 = '<img class="kw-prodimage-img-secondary" src="'.$image2Url.'" alt="'.esc_attr( get_post_meta($secondary_image_id, '_wp_attachment_image_alt', true) ).'" title="'.esc_attr( get_the_title($secondary_image_id) ).'">';
						}
					}
...
?>
	<div class="details kw-details fixclear">
		<h3 class="kw-details-title text-custom-child"><?php the_title(); ?></h3>
<?php
}
...
function zn_woocommerce_single_product_image_html($html) {
	...
	if('yes' === get_option( 'woocommerce_enable_lightbox' )){
	    $html = str_replace('data-rel="prettyPhoto[product-gallery]"', 'data-shop-mfp="image"', $html);
	}
    return $html;
}
...
function zn_add_main_image_to_gallery( $ids ){
	$thumbs_behavior = zget_option( 'zn_show_thumb_on_hover', 'zn_woocommerce_options', false, 'yes' );
	if( $thumbs_behavior == 'click' || $thumbs_behavior == 'yes' ){
