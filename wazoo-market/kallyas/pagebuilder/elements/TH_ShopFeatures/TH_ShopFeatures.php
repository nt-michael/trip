					...
					if ( isset ( $options['sf_single'] ) && is_array( $options['sf_single'] ) ) {
						foreach ( $options['sf_single'] as $single ) {
							...
							if ( ! empty ( $single['lp_single_logo'] ) ) {
								echo $lp_link['start'] . '<img src="' . $single['lp_single_logo'] . '" alt="'. ZngetImageAltFromUrl( $single['lp_single_logo'] ) .'" title="'.ZngetImageTitleFromUrl( $single['lp_single_logo'] ).'">' . $lp_link['end'];
							}
