...
class TH_StaticContentShowroomCarousel extends ZnElements
{
	...
	function element()
	{
		...
		?>
		<div class="kl-slideshow static-content__slideshow sc--showroom-carousel <?php echo $style; ?> <?php echo $uid; ?> <?php echo $bm_class ?> <?php echo $scr_main_class; ?> <?php echo zn_get_element_classes($this->data['options']); ?>">
	...
	foreach ( $options['single_shcar'] as $item ):
		...
		if( isset($item['sc_shcar_image']) && !empty($item['sc_shcar_image']) ){
			...
			echo '<a href="'.$linkurl.'" class="shc__item-link" data-lightbox="'.$datalightbox.'">';
				...
				echo '<img src="'.$image['url'].'" alt="'. ZngetImageAltFromUrl( $item['sc_shcar_image'] ) .'" title="'.ZngetImageTitleFromUrl( $item['sc_shcar_image'] ).'" class="img-responsive">';
			echo '</a>';
		}
