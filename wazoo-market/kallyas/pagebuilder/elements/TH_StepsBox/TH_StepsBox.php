			...
			if ( ! empty ( $options['steps_single'] ) && is_array( $options['steps_single'] ) ) {
				foreach ( $options['steps_single'] as $step ) {
					...
					if ( $stp_icontype == 'img' && ! empty ( $step['stp_single_icon'] ) ) {
						...
						echo '<img ' . $animation . ' src="' . $step['stp_single_icon'] . '" alt="'. ZngetImageAltFromUrl( $step['stp_single_icon'] ) .'" title="'.ZngetImageTitleFromUrl( $step['stp_single_icon'] ).'" class="process_steps__step-icon-src">';
						echo '</div>';
					}
