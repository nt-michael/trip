...
/*
 ...
 Keywords: video button, modal video, player, youtube, vimeo, self hosted, embed, embedded
 Level: 3
*/
/**
 * Class TH_VideoBox
	...
	function element()
	{
		...
		if($el_type == 'modal'){
			...
			if ( ! empty ( $options['vb_video_image'] ) && ! empty ( $options['vb_video_url'] ) ) {
				...
				echo '<div class="adbox video '.implode(' ', $classes).'" '.$attributes.'>';
					...
					echo '<img class="adbox-img" src="' . $image['url'] . '" alt="'. ZngetImageAltFromUrl( $options['vb_video_image'] ) .'" title="'.ZngetImageTitleFromUrl( $options['vb_video_image'] ).'">';
		...
		elseif ($el_type == 'player'){
			...
			if( ( $vb_video_source == 'external_yt' || $vb_video_source == 'external_vim' || $vb_video_source == 'external_other' ) && !empty($external_url)){
				...
				echo '<div class="video-ext-wrapper fitvids-resize-wrapper">'.get_video_from_link( $external_url, '', '', '', $video_attributes ) . '</div>';
