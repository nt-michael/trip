							...
							if ($this->opt($type) == 1) {
								...
								if ($layout == 'tabs') {
									...
									echo '<div class="col-sm-12">';
										echo '<h3 class="m_title m_title_ext text-custom ff-alternative spp-title spp-el-title">' . $name['wtitle'] . '</h3>';
									echo '</div>';
									echo '<div class="col-sm-12">';
								}
								...
								echo '<div class="shop-latest-carousel spp-carousel '. ( $isCarousel ? 'spp-carousel--enabled': 'spp-carousel--disabled' ) .'">';
									...
									if ($product_query->have_posts()) {
										echo '<ul class="featured_products spp-carousel-list spp-list--cols-'.$name['wcols'].' '.( $isCarousel != 1 ? 'clearfix': '' ).'" data-autoplay="'.$name['wauto'].'" data-timeout="'.$name['wtimeout'].'" >';
		...
		$options = array(
			...
			'featured' => array(
				...
				'options' => array(
					...
					array (
						'name'        => __( 'Products per row', 'zn_framework' ),
						'description' => __( 'Please select how many products per row to display.', 'zn_framework' ),
						'id'          => 'woo_fp_perrow',
						'std'         => '4',
						'type'        => 'select',
						'options'        => array(
