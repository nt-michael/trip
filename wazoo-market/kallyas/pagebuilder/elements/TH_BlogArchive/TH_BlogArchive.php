...
class TH_BlogArchive extends ZnElements
{
	...
	function element()
	{
		...
		echo '<div class="zn_blog_archive_element '.implode(' ', $classes).'" '.$attributes.'>';
			...
			if ( $zn_config['blog_layout'] == 'cols' || in_array( $zn_config['blog_columns'], array(2, 3, 4) ) ) {
			...
			'general' => array(
				...
				'options' => array(
					...
					array (
					    ...
					    "type"        => "select",
					    "options"     => array (
					        'def_classic' => __( "Generic Blog - Classic Style", 'zn_framework' ),
					        'def_modern' => __( "Generic Blog - Modern Style", 'zn_framework' ),
					        'cols' => __( "Multi-columns blog", 'zn_framework' ),
					    ),
					),
