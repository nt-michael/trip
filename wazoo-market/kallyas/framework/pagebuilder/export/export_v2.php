...
class ZnPbExportHelper{
	...
	function zn_import_template(){
		...
		$content = array(
			...
			// @since v4.1.4
			// Fixes bug when the custom javascript was not imported from an exported template
			'custom_js' => $custom_js,
		);
		...
		// error_log( 'USED MEMORY: '. $used_memory );
		// error_log( 'USED time ( seconds ): '. date('s', $used_time) );
		// error_log( 'USED time ( microseconds ): '. $used_time );
	...
	function zn_export_template(){
		...
		// Generate the template zip file here
		// $start_memory = memory_get_usage();
		// $start_time = microtime(true);
