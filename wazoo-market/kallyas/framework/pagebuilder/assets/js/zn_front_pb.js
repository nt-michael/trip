		...
		sortable_arguments : function( scope ) {
			...
			return {
				...
				stop : function( event, ui ) {
					...
					if (ui.item.closest('.zn_pb_wrapper').length === 0) {
						return true;
					}
			...
			$('.zn_pb_export_template').on('click', function(e){
				...
				jQuery.post( ZnAjax.ajaxurl, data, function( response ) {
					...
					if ( response.success === true ) {
						...
						window.showed_message = true;
						location.href = ZnAjax.ajaxurl+"?action=znpb_download_export&nonce=" + ZnAjax.security;
					}
					else{
 ...
 */
		...
		make_draggable : function(){
			...
			$( ".zn_pb_element" ).draggable(fw.get_draggable_options());

			$( document ).keyup( function( e ) {
				if( e.which === 27 || e.keyCode === 27 ) {
					$( '.ui-draggable-dragging' ).addClass('znpb_cancel_drop').trigger('mouseup');
				}
			});
			...
			element.click(function(e){
				...
				jQuery.post( ZnAjax.ajaxurl, data, function( response ) {
					...
					if ( response.message ) {
						...
						saved_el.find('.zn_pb_element').draggable(fw.get_draggable_options());
			...
			fw.scope.on( 'click', '.znpb-element-options-trigger', function(e){
				...
				params.modal_on_ajax_load = function(e){
					...
					function save_order(sort_option, initial_setup){
						...
						console.log( 'New order', fw.stop_order );
					}
