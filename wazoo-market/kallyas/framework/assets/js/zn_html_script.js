...
(function($)
{
	...
	$.ZnHtml.prototype = {
		...
		radio_image : function(scope){
			...
			var btn = element.find('.zn-radio-button:not([data-dummy])'),
				select = element.find('.zn-radio-image-select');
		...
		smart_select : function(scope){
			...
			var btn = element.find('.zn-smartselect-label'),
				items = element.find('.zn-smartselect-item'),
				select = element.find('.zn-smartselect-selectlist');
		...
		boxmodel : function(scope){
			...
			element.each(function(index, el) {
				...
				fields.on('change keyup', function(event) {
					...
					var curTraget = $(event.currentTarget),
						ctv = curTraget.val().trim(),
						ctv_numer = parseInt(ctv),
						ctv_unit = ctv.replace(ctv_numer, ''),
						ctv_step = 1;
					...
					switch(event.which) {
						 // up
				        case 38:
				        	ctv = ( ctv_numer + ctv_step ) + ctv_unit;
				        	curTraget.val( ctv );
				        break;
				         // down
				        case 40:
				    ...
				    event.preventDefault();
				    // update others if linked is enabled.
		...
		launch_ui_slider : function( scope ) {
			...
			$(element).each(function() {
				...
				var el = $(this),
					...
					disabled = jQuery(this).is('[data-disabled="true"]') ? true : false;
				...
				jQuery( this ).slider({
					...
					slide: function( event, ui ) {
						jQuery( this ).siblings('.wp-slider-input').val( ui.value ).trigger('change');
					}
				});
